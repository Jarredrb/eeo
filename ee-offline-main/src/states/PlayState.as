package states
{
	...
	public class PlayState extends BlState
	{
		...
		private function getLayerFromId(id:int):int {
			return (id >= 500 && id < 1000) || (id >= 5000 && id < 6000) ? 1 : 0;
		...
		public function addFakePlayer(smiley:int, aura:int, auraColor:int, goldBorder:Boolean = false, xx:int = -1, yy:int = -1, name:String = "_fp_", admin:Boolean = false, adminRow:int = 0):void {
			var id:int = fPid_++;
			...
			p = new Me(world, name == "_fp_" ? "fake" + num.toString() : name, false, this);
			players[id] = p;
			p.id = id;
