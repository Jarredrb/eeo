	...
	public class World extends BlTilemap
	{
		...
		public function setTileComplex(layer:int,x:int, y:int, type:int, properties:Object):void
		{
			...
			switch(type){
				...
				case ItemId.DOOR_ORANGE:
				case ItemId.GATE_ORANGE:
				case ItemId.SPIKE_DOOR_PURPLE:
				case ItemId.SPIKE_GATE_PURPLE:
				case ItemId.SPIKE_DOOR_ORANGE:
				case ItemId.SPIKE_GATE_ORANGE:
				case ItemId.WORLD_PORTAL_SPAWN: {
					lookup.setInt(x, y, properties.rotation);
					break;
				}
			...
			for(var cy:int=oy; cy < ow; cy++){
				...
				for(var cx:int=ox; cx < oh; cx++){
					...
					switch(val){
						...
						case ItemId.SPIKE_DOOR_PURPLE: if ( pl.switches[lookup.getInt(cx, cy)]) continue; break;
						case ItemId.SPIKE_GATE_PURPLE: if (!pl.switches[lookup.getInt(cx, cy)]) continue; break;
						
						case ItemId.SPIKE_DOOR_ORANGE: if ( orangeSwitches[lookup.getInt(cx, cy)]) continue; break;
						case ItemId.SPIKE_GATE_ORANGE: if (!orangeSwitches[lookup.getInt(cx, cy)]) continue; break;
			...
			for( cy=starty;cy<endy;cy++){
				...
				for( cx=startx;cx<endx;cx++){
					...
					switch(type){
						...
						case ItemId.SPIKE_DOOR_PURPLE: {
							if (player.switches[lookup.getInt(cx, cy)]) {
								ItemManager.sprSpikePurpleDoorDOWN.drawPoint(target, point,lookup.getInt(cx, cy))
							} else {
								ItemManager.sprSpikePurpleDoorUP.drawPoint(target, point,lookup.getInt(cx, cy))
							}
							continue;
						}
						case ItemId.SPIKE_GATE_PURPLE: {
							if (player.switches[lookup.getInt(cx, cy)]) {
								ItemManager.sprSpikePurpleGateDOWN.drawPoint(target, point,lookup.getInt(cx, cy))
							} else {
								ItemManager.sprSpikePurpleGateUP.drawPoint(target, point,lookup.getInt(cx, cy))
							}
							continue;
						}
						...
						case ItemId.SPIKE_DOOR_ORANGE: {
							if (orangeSwitches[lookup.getInt(cx, cy)]) {
								ItemManager.sprSpikeOrangeDoorDOWN.drawPoint(target, point,lookup.getInt(cx, cy))
							} else {
								ItemManager.sprSpikeOrangeDoorUP.drawPoint(target, point,lookup.getInt(cx, cy))
							}
							continue;
						}
						case ItemId.SPIKE_GATE_ORANGE: {
							if (orangeSwitches[lookup.getInt(cx, cy)]) {
								ItemManager.sprSpikeOrangeGateDOWN.drawPoint(target, point,lookup.getInt(cx, cy))
							} else {
								ItemManager.sprSpikeOrangeGateUP.drawPoint(target, point,lookup.getInt(cx, cy))
							}
							continue;
						}
